module.exports= {
    ...
    getBackground: (req, res, next) => {
        ...
        const dbInstance = req.app.get('db');
        dbInstance.get_background([req.params.id]).then(background => res.status(200).json(background));
    },
    setBackground: (req, res, next) => {
        console.log(req.body);
        const dbInstance = req.app.get('db');
        dbInstance.set_background([req.body.board_id, req.body.url]).then(background => res.status(200).json(background));
    }    
}